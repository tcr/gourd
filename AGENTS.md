...
- Prelude: `gourd::prelude::*` (GoMutex, GoRc, GoError, Any, etc.)
- Packages: `gourd::packages::*` (os_open, strings_replace, etc.)

### gourd-codegen transpiler

The transpiler is organized in `gourd-codegen/src/transpiler/` with three layers:
...
| `heuristics.rs` | Heuristic detection for ambiguous type constructs |
...
| `hir/mod.rs` | New HIR-based transpiler (AST → HIR → Rust tokens) |
| `legacy/mod.rs` | Legacy flat-expr transpiler (old code, maintained for compatibility) |
...
## Root-level runtime primitives (`go_gc.rs`, `go_scheduler.rs`)
...
### Prelude modules (`gourd::prelude::*`)

Prelude provides runtime types for user code (not generated by the transpiler):
...
| `std` | `len`, `cap`, `append`, `make_slice`, `make_map`, `map_get`, `map_set`, `copy`, `min`, `max`, `std_copy`, `std_delete`, `std_append` (map operations, formatting helpers) |
...
| File | Functions |
|------|-----------|
| `os_impl.rs`, `os.rs` | 12 os functions |
| `strings_ops.rs`, `strings_impl.rs`, `strings.rs` | 23 strings functions |
...
| `time_impl.rs`, `time.rs` | 5 time functions |
