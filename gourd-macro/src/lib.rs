//! gourd-macro: procedural macro crate for Go → Rust transpilation.
//!
...
//! use the `gourd-codegen` crate directly.
...
pub fn go(input: TokenStream) -> TokenStream {
    ...
    gourd_codegen::transpile_go(tokens).into()
}
...
pub fn verify_rust_output(attr: TokenStream, input: TokenStream) -> TokenStream {
    ...
    gourd_codegen::verify_short(attr_tokens, input_tokens).into()
}
