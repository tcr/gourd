...
go! {
    func goCountPositive(data []int) int {
        count := 0
        for i := range data {
            if data[i] > 0 {
                count = count + 1
...
fn test_for_range_single() {
    let data = vec![1, -2, 3, -4, 5];
    let result = goCountPositive(&data);
